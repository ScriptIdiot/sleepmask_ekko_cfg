BOOL markCFGValid_nt(PVOID pAddress)
{
	...
	NTSTATUS status = NTDLL$NtQueryVirtualMemory(NtCurrentProcess(), pAddress, MemoryBasicInformation, &mbi, sizeof(mbi), 0);
	...
	ntStatus = NTDLL$NtSetInformationVirtualMemory(NtCurrentProcess(), VmCfgCallTargetInformation, 1, &tVirtualAddresses, (PVOID)&tVmInformation, (ULONG)sizeof(tVmInformation));
